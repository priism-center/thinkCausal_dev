...
store_l_potential_outcomes$quiz <- function(){
  ...
  question_prompt_1 <- sortable::bucket_list(
    ...
    group_name = store_l_potential_outcomes$ns_quiz('answers'),
    orientation = "horizontal",
    class = 'default-sortable sortable-wide',
    sortable::add_rank_list(
      input_id = store_l_potential_outcomes$ns_quiz('answers_variables'),
      text = strong("Available"),
      labels = c('bp_3month', 'sex', 'height'),
      options = sortable::sortable_options(multiDrag = TRUE)
    ),
    sortable::add_rank_list(
      input_id = store_l_potential_outcomes$ns_quiz('answers_include'),
      text = strong("Control for"),
      labels = NULL,
      options = sortable::sortable_options(multiDrag = TRUE)
    ),
    sortable::add_rank_list(
      input_id = store_l_potential_outcomes$ns_quiz('answers_treatment'),
      text = strong("Treatment"),
      labels = c('fish_oil'),
      options = sortable::sortable_options(disabled = TRUE)
    ),
    sortable::add_rank_list(
      input_id = store_l_potential_outcomes$ns_quiz('answers_outcome'),
      text = strong("Outcome"),
      labels = c('bp_6month'),
      options = sortable::sortable_options(disabled = TRUE)
    )
  ...
  question_prompt_2 <- radioButtons(
    inputId = store_l_potential_outcomes$ns_quiz('answers'),
