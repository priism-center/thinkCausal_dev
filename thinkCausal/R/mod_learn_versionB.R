...
mod_learn_versionB_ui <- function(id){
  ns <- NS(id)
  tagList(
    shinyjs::useShinyjs(),
...
#'
#' @noRd
mod_learn_versionB_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    output$scroll_visual <- renderUI({
      ...
      dat <- reactive({
        if (input$view == 'What happened' & input$imputed == FALSE) {
        ...
        if (input$view == 'What happened' & input$imputed == TRUE) {
          dat <- imputed
          names(dat)[names(dat) == 'ITE'] <- 'est.ITE'
        }

        if(input$view == 'What would have happened?') {
          dat <- dplyr::mutate(truth,
        ...
        if(input$view == 'Compare' & input$imputed == FALSE) {
          dat <- truth
        }

        if(input$view == 'Compare' & input$imputed == TRUE) {
          dat <- combined
        }
      ...
      background <- reactive({
        hyperShoe <- switch (input$view,
                             'What happened' = 'white',
                             'What would have happened?' = 'black',
                             'Compare' = 'white'
        )

        Y0 <- switch (input$view,
                      'What happened' = switch (as.character(input$imputed),
                                             'TRUE' =  ifelse(truth$hyperShoe == 1, imputedY0, 'white'),
                                             'FALSE' = 'white'),
                      'What would have happened?' = rep('black', nrow(truth)),
                      'Compare' = ifelse(dat()[['hyperShoe']] == 1, 'black', 'white')
        )
        ...
        Y1 <- switch (input$view,
                      'What happened' = switch (as.character(input$imputed),
                                             ...
                                             'FALSE' = 'white'),
                      'What would have happened?' = rep('black', nrow(truth)),
                      'Compare' = ifelse(dat()[['hyperShoe']] == 0, 'black', 'white')
        )
      ...
      items$position5 <- div(
        ...
        renderUI({
          shinyWidgets::radioGroupButtons(ns('view'), label = 'select a view:',
                                          choices = c('What happened', 'What would have happened?', 'Compare'),
                                          individual = T
          )
        }),
        renderUI({
        ...
        reactable::renderReactable({
          if(input$imputed == TRUE & input$view == 'Compare'){
          ...
          }else if(input$imputed == FALSE & input$view == 'Compare'){
      ...
      items$position6 <- div(
        ...
        renderUI({
          shinyWidgets::radioGroupButtons(ns('view6'), label = 'select a view:',
                                          choices = c('What happened', 'Compare'),
                                          individual = T,
                                          selected = 'Compare'
          )
        }),
        renderImage({
          if(input$view6 == 'Compare') path <- 'www/learn/fundemental/plots/cf1.png'
          if(input$view6 != 'Compare') path <- 'www/learn/fundemental/plots/factual1.png'
      ...
      items$position7 <- div(
        ...
        renderUI({
          if(input$viz7 == 'Plot'){
            renderImage({
              if(input$view7 == 'Compare') path <- 'www/learn/fundemental/plots/cf2.png'
              if(input$view7 != 'Compare') path <- 'www/learn/fundemental/plots/factual2.png'
              list(src = app_sys('app', path),
                   contentType = 'image/png',
                   width = 800,
                   height = 500)
