...
mod_learn_test_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    output$scroll_visual <- renderUI({
      ...
      # visual 1
      items$p1 <- renderCachedPlot(ggplot2::qplot(1:10) + ggplot2::labs(title = 'Section 1'),
                                   cacheKeyExpr = { list(1) })

      # visual 2
      items$p2 <- div(
        renderCachedPlot(ggplot2::qplot(1:10, 1:10) + ggplot2::labs(title = 'Section 2'),
                         cacheKeyExpr = { list(2) }),
        renderTable(data.frame(x = 1:4, y = 3:6, z = 5:8))
      )

      # visual 3
      items$p3 <- renderCachedPlot(ggplot2::qplot(rnorm(10), 1:10) + ggplot2::labs(title = 'Section 3'),
                                   cacheKeyExpr = { list(3) })

      return(items)
    })

    # must activate the scrollytell
