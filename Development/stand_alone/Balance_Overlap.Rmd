...
plot_overlap <- function(df, nooverlap = FALSE){
  ...
  if(nooverlap == TRUE){
    ggplot() + 
      ...
      scale_fill_manual("Treated?", values=c("blue","red")) + 
...
```{r, echo=FALSE}
dgp_overlap <- function(n, tau, lwr, upp, seed, nooverlap = FALSE){
  set.seed(seed)
  ...
  Y0 <- X + rnorm(n = n, mean = 0, sd = 1)
  Y1 <- X + tau + rnorm(n = n, mean = 0, sd = 1)
  Y <- ifelse( Z == 1, Y1, Y0)
  return(data.frame( X, Y, Y0, Y1, Z))
}

df_overlap1 <- dgp_overlap(100, 5, 0.05, 0.95, 123)
df_overlap2 <- dgp_overlap(100, 5, 0.05, 0.95, 4)
df_overlap3 <- dgp_overlap(100, 5, 0.5, 0.65, 1, nooverlap = TRUE)
```
...
df_nonoverlap_unbalance <- dgp_nonoverlap_unbalance(1000, 1234)
model <- lm(Y ~ X + as.factor(Z), data = df_nonoverlap_unbalance)
df_nonoverlap_unbalance$fitted <- model$fitted.values
...
ggplot() + geom_point(data = df_nonoverlap_unbalance[df_nonoverlap_unbalance$Z == 1, ], aes(x = X, y = Y, color = "1")) + 
  ...
  geom_line(data = df_nonoverlap_unbalance[df_nonoverlap_unbalance$Z == 0, ], aes(x = X, y = fitted), linetype = "dashed") + 
  geom_line(data = df_nonoverlap_unbalance[df_nonoverlap_unbalance$Z == 1, ], aes(x = X, y = fitted), linetype = "dashed") +
  theme(legend.position="bottom")
...
The OLS estimator fails to capture the true effect. There just isn't enough information in the data to extrapolate fully into areas where there isn't overlap. Using flexible models such as BART would be the way to go.
```{r}
model1 <- lm(Y~as.factor(Z) + X, data = df_nonoverlap_unbalance)
summary(model1)
```
