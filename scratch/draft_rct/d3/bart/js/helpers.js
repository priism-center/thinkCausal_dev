...
bart.generateData = function(){
  ...
  interpData.map((d, i) => d.age = newX[i] )
  interpData.map((d, i) => d.runningTime = newY[i] )

  // add noise
  interpData.map( (d,i) => bart.clamp( d.age = +d.age + noiseX[i], rangeX[0], rangeX[1] ))
...
bart.onMouseMove = function() {
  ...
  const mouseRange = [
    xScale(d3.min(bart.data.observations, d => d.age)),
    xScale(d3.max(bart.data.observations, d => d.age))
  ]
  mouseX = bart.clamp(mouseX, mouseRange[0], mouseRange[1]); 

  // recover coordinate from the fitted line
  const data = Array.from(bart.data.fits, d => d.age);
